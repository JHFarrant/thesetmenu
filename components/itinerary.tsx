...
import { Card, Spinner, Badge, ToggleSwitch, Tooltip, Button, Drawer, Header, Items, Blockquote} from "flowbite-react";
import { HiInformationCircle, HiOutlineChatAlt2, HiOutlinePaperAirplane } from "react-icons/hi";
...
const Itinearry = ({
  ...
  const toggleSelectedState = (id) => {
    ...
    const nextState = ((currentState ?? "unselected") == "unselected") ? "selected" : "unselected";
    setSelectedEvents({...selectedEvents,
                    [id]: nextState});
  }

  const atLeastOneSelected = Object.values(selectedEvents).some(e => e == "selected")
...
{/*      {!!setRecommendationsEnabled && (
                  ...
                  return (
                    ...
                    >
                      <div id="artistListItem" className="flex space-x-2 p-1">
                        ...
                        <div id="artistInfo" className="flex flex-col space-y-2 items-left w-full justify-between">
                          {/* <div className='flex' > */}
                        ...
                        <div className="flex w-full justify-between items-center space-x-2">
                          <div id="spotifyIconArtistLink">
                            <a href={!demoMode && favourite.artist.external_urls.spotify}>
                              <img
                                alt={favourite.artist.name}
  ...
  <h2 className={`text-m font-semibold text-center`}>
    ...
    { !demoMode && <Drawer open={showShareDrawer} onClose={()=>{setShowShareDrawer(false)}} position="bottom" backdrop={false} className="bg-wa-background p-2" edge={true} theme={{ edge: "bottom-16" }}>
        <Drawer.Items>
          <div className="mb-2 inline-flex items-center text-base font-semibold text-gray-500 dark:text-gray-400"><h3>Share with Whatsapp</h3></div>
          ...
          <Button pill color="gray" onClick={()=>{window.open(generateWhatsappTextLink(itineraryInDays, selectedEvents))}}>
...
const generateWhatsappTextLink = (itineraryInDays, selectedEvents) => {
  ...
  const date = shiftedDay(dailyItinerary[0]?.start)
  const fiteredDailyItinerary = dailyItinerary.filter(e => selectedEvents[e.id] && selectedEvents[e.id] == "selected" )
  if(fiteredDailyItinerary.length){
    const shortDate = date?.format("dddd")
    whatsappTextLink += (emojis[shortDate]+" "+shortDate+" "+emojis[shortDate]+"\n")
