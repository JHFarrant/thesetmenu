...
const getEventString = (event) => {
  return  <div className="border-l-4 border-gray-600 pl-2 mt-1">
      <span className="font-mono">{event.start.format("h:mma")}</span> 
      {" "}{event.name}{" @ "}<i>{event.location}</i>
      </div>
...
return <>
<p key={"MyGlastoSetMenuTitle"}>📀 <span className="text-blue-500 underline">MyGlastoSetMenu.co.uk</span></p>
{ 
  ...
  if(fiteredDailyItinerary.length){
    return <div key={date?.format("dddd")}>
            <p>{emojis[date?.format("dddd")]} <b>{date?.format("dddd")}</b> {emojis[date?.format("dddd")]}</p>
            {
                fiteredDailyItinerary.map((event: any) => {
                  return <div  key={event.id}><Typing
                  speed={10}
                  >
